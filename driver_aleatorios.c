...
static ssize_t ler_driver(struct file *arquivo, char __user *buffer_usuario, size_t tamanho_maximo, loff_t *deslocamento){
    ...
    // if == 1 -> erro
    if(copy_to_user(buffer_usuario, &seed, num_bytes_copiados)){ // coloca o número no buffer do usuário
        return -EFAULT;
    }
...
static int __init inicializar(void) {
    // Registra dispositivo com número major dinâmico
    id_driver = register_chrdev(0, NOME, &operacoes);
    if (id_driver < 0) {
        return id_driver;
    }
